...
for I0 in light_intensities
    ...
    HCO3, CO3, HPO4, PO4 = zeros(nz), zeros(nz), zeros(nz), zeros(nz)
    pop[1:25] .= X0
    ...
    # @time for time_step in 1:2
        ...
        for i_inner in 1:tp.n_inner
            ...
            lowH2PO4, diagH2PO4, upH2PO4 = getdiagonals_ions(ip.D_H2PO4, dz, tp.dt, ind)
            ...
            # if time_step == 1 && i_inner == tp.n_inner
            #     sol_H = compute_pH(gp.KI, gp.KII, ip.QI, ip.QII, ip.kw, CO2, NO3, H2PO4, ip.C_Na, ip.C_K, ind)
            #     println("CO2 :", CO2[1:30])
            #     println("H2PO4 :",H2PO4[1:30])
            #     println("NO3 :",NO3[1:30])
            #     println("solH = ", sol_H[3])
            # end
            
        end
        ind = findfirst(x -> x == 0, pop) - 1
        sol_H = compute_H(gp.KI, gp.KII, ip.QI, ip.QII, ip.kw, CO2, NO3, H2PO4, ip.C_Na, ip.C_K, ind)
            # if i_inner == 1
            #     if length(sol_pH) != 3
            #     println((length(sol_pH)))
            #     end
            # end
        H = pad_zeros(sol_H[3],nz)
        pH = -log10.(H./1000)
        ...
        df_H[!,"$time_step"] = copy(H)
        df_pH[!,"$time_step"] = copy(pH)
    end
